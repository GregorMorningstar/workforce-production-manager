...
<head>
    ...
    <script>
        window.addEventListener('DOMContentLoaded', () => {
            ...
            const tick = () => {
                ...
                if (circle) circle.style.strokeDashoffset = (1 - ratio) * 283;
                if (left <= 0) {
    ...
    <style>
        .countdown-circle { transform: rotate(-90deg); transform-origin: center; }
    </style>
</head>
<body class="h-full bg-slate-900 text-slate-100 flex items-center justify-center p-6">
<div class="w-full max-w-md space-y-6">
    <div class="text-center space-y-3">
        <h1 class="text-3xl font-semibold">404 – Nie znaleziono</h1>
        <p class="text-slate-400 text-sm">
            Szukana strona nie istnieje.
            @if($role)
                (Twoja rola: <span class="font-medium">{{ $role }}</span>)
            @endif
        </p>
        <p class="mt-2">
            Za <span id="seconds" class="font-mono text-lg">{{ $seconds }}</span> s nastąpi przekierowanie.
        </p>
    </div>
    ...
    <div class="flex items-center justify-center">
        <svg width="140" height="140">
            ...
            <circle id="circle" class="countdown-circle transition-all duration-1000 ease-linear"
                    cx="70" cy="70" r="45"
